...
void ModulePhysics3D::AddConstraintHinge(PhysBody3D& bodyA, PhysBody3D& bodyB, const vec3& anchorA, const vec3& anchorB, const vec3& axisA, const vec3& axisB, bool disable_collision)
{
	...
	hinge->enableAngularMotor(true, 1, 2000000);
	
	
}


void ModulePhysics3D::AddConstraintFixed(btRigidBody& rbA, btRigidBody& rbB, const btTransform& frameInA, const btTransform& frameInB, bool disable_collision)
{
	btFixedConstraint* fix = new btFixedConstraint(rbA, rbB, frameInA, frameInB);
	world->addConstraint(fix, disable_collision);
	constraints.add(fix);
	fix->setDbgDrawSize(2.0f);
}
...
void ModulePhysics3D::Reset(PhysVehicle3D* v)
{
	v->body->setAngularVelocity(btVector3(0, 0, 0));
	v->body->setLinearVelocity(btVector3(0, 0, 0));
	v->SetPos(0, 12, 25);
	v->SetTransform(App->player->matrix);
